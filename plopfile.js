module.exports = function (plop) {
    plop.setGenerator('swagger', {
        description: 'Generate TypeScript interfaces from a Swagger specification',
        prompts: [
            {
                type: 'input',
                name: 'url',
            ...
            },
            {
                ...
                default: './src/api/models'
            }
        ],
        actions: function(data) {
            // const spec = await axios.get(data.url).then(res => res.data);
            const swagger = {
                "openapi": "3.0.1",
                "info": {
            ...
            const parsedSchemas = typeKeys.map(schemaKey => {
                ...
                return {
                    name: schemaKey,
                    type: schemaType,
                    data: swagger.components.schemas[schemaKey]
                };
            });
            ...
            parsedSchemas.forEach(schemaData => {
                ...
                if (schemaData.type !== 'enum') {
                    ...
                    // const parsed = schemaData.name;
                    const {propertiesContent, refs} = transformProperties(!!schemaData.data.properties ? schemaData.data.properties : {}, swagger);
                    genConfig.push({
                        type: 'addMany',
                        destination: data.output,
                        base: 'templates',
                        templateFiles: `templates/\\{{dashCase name}}.ts.hbs`,
                        data: {
                            name: schemaData.name,
                            properties: propertiesContent
                        }
                    })
                    // const importsContent = transformRefsToImport(refs, `${options.path}/core`, `${parsed.path}/${dasherize(parsed.name)}`);
                }
            });
            // Use the Swagger spec to generate TypeScript interfaces
            return genConfig;
        }
    });
};
